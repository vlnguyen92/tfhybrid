...
from ..nets import alexnethybrid, vgghybrid, overfeathybrid
...
net_configs = {
    ...
    '2dev-model': (attrgetter('ndev_model'),
                   ['/job:worker/task:0/gpu:0', '/job:worker/task:1/gpu:0'],
                   'grpc://localhost:2222',
                   2, 48),
    '2dev-data': (attrgetter('ndev_data'),
                  ...
                  'grpc://localhost:2222',
                  2, 48),
    '2dev-model-data': (attrgetter('ndev_model_data'),
                        ['/job:worker/task:0/gpu:0', '/job:worker/task:1/gpu:0',
                         '/job:ps/task:0/cpu:0'],
                        'grpc://localhost:2222',
                        2, 48),
    '4dev-model': (attrgetter('ndev_model'),
                   ['/job:worker/task:0/gpu:0', '/job:worker/task:1/gpu:0',
                    '/job:worker/task:2/gpu:0', '/job:worker/task:3/gpu:0'],
                   'grpc://localhost:2222',
                   4, 24),
    '4dev-data': (attrgetter('ndev_data'),
                  ['/job:worker/task:0/gpu:0', '/job:worker/task:1/gpu:0',
                   '/job:worker/task:2/gpu:0', '/job:worker/task:3/gpu:0',
                   '/job:ps/task:0/cpu:0'],
                  'grpc://localhost:2222',
                  4, 24),
    '4dev-model-data': (attrgetter('ndev_model_data'),
                        ['/job:worker/task:0/gpu:0', '/job:worker/task:1/gpu:0',
                         '/job:worker/task:2/gpu:0', '/job:worker/task:3/gpu:0',
                         '/job:ps/task:0/cpu:0'],
                        'grpc://localhost:2222',
...
net_names = {
    'alexnet': alexnethybrid,
    'vgg': vgghybrid,
    'overfeat': overfeathybrid
}
...
def evaluate(net_configname, network_name, batch_size, devices=None, target=None,
    ...
    with tf.Graph().as_default():
        ...
        network = config[0](net_names[network_name])
        if devices is None:
            devices = config[1]
        if target is None:
            target = config[2]
        ...
        correct_op = network(images, labels, num_classes, None, devices, is_train=False)
        ...
        with tf.Session(target, config=config) as sess:
            ...
            tf.train.SummaryWriter(tb_dir, sess.graph)
...
def train(net_configname, network_name, batch_size, devices=None, target=None,
    ...
    with tf.Graph().as_default():
        ...
        (net, logprob, total_loss,
         train_op, global_step) = network(images, labels, num_classes,
        ...
        with tf.Session(target, config=config) as sess:
            ...
            summary_writer = tf.train.SummaryWriter(tb_dir, sess.graph)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--network", help='the name of the parallel method',
                        choices=['alexnet', 'vgg', 'overfeat'],
                        default='alexnet')
    parser.add_argument("--mode", help='the name of the parallel method',
                        choices=['1dev', '2dev-data', '2dev-model', '2dev-model-data',
                                 '4dev-data', '4dev-model', '4dev-model-data'],
                        default='2dev-model-data')
    ...
    if args.redirect_outerr:
        with open(out_file, 'w') as f, misc.stdout_redirected(f):
            with open(err_file, 'w') as f, misc.stdout_redirected(f, stdout=sys.stderr):
                if args.eval:
                    evaluate(args.mode, args.network, tb_dir=args.log_dir, train_dir=args.model_dir,
                             benchmark_name=args.dataset, batch_size=args.batch_size)
                else:
                    train(args.mode, args.network, tb_dir=args.log_dir, train_dir=args.model_dir,
