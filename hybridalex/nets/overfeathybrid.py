...
from .overfeatcommon import overfeat_part_conv, overfeat_inference, overfeat_loss, overfeat_eval
...
def original(images, labels, num_classes, total_num_examples, devices=None, is_train=True):
    ...
    with tf.device(devices[0]):
        ...
        net, logits, total_loss = overfeat_inference(builder, images, labels, num_classes)

        if not is_train:
            return overfeat_eval(net, labels)
...
def ndev_data(images, labels, num_classes, total_num_examples, devices, is_train=True):
    ...
    if not is_train:
        with tf.variable_scope('model'):
            prob = overfeat_inference(builder, images, labels, num_classes)[0]
        return overfeat_eval(prob, labels)
        ...
        with tf.name_scope(''):
            for idx in range(len(devices)):
                ...
                with tf.name_scope('tower_{}'.format(idx)) as scope:
                    with tf.device(dev):
                        prob, logits, total_loss = overfeat_inference(builder, image_slices[idx],
                                                                      label_slices[idx],
                                                                      num_classes, scope)
...
def ndev_model(images, labels, num_classes, total_num_examples, devices, is_train=True):
    ...
    with tf.device(devices[0]):
        total_loss = overfeat_loss(logits, labels)
...
def ndev_model_data(images, labels, num_classes, total_num_examples, devices, is_train=True):
        ...
        with tf.name_scope(''):
            for idx in range(len(devices)):
                ...
                with tf.name_scope('tower_{}'.format(idx)), builder.device(dev):
                    net = overfeat_part_conv(builder, image_slices[idx])
    ...
    with tf.variable_scope('model'):
        ...
        with tf.name_scope(''):
            with builder.parallel(devices, colocate_variables=True):
                net = builder.fc('fc3072', conv_outputs, 3072, concat_axis=0,
                                 activation=tf.nn.relu)
            ...
            for idx in range(len(devices)):
                with builder.device(devices[idx]):
                    l = overfeat_loss(logit_slices[idx], label_slices[idx])
